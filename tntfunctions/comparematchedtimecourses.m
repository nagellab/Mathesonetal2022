function comparematchedtimecourses(atlaspath,drivers,savefigs,type)
...
for k=1:numel(drivers)
    ...
    if(strcmp(type,'silencing'))
        resdata=genotypedata.res10salwaysonalwaysoff;
        ctrlresdata=ctrldata.res10salwaysonalwaysoff;
    elseif(strcmp(type,'activation'))
        %not really res and control but just to get two conditions
        resdata=genotypedata.resblankalwayson10s;
        ctrlresdata=ctrldata.resblankalwayson10s;
    end

    plotcomparevels_multpretty({'pmove','upwindvelocity','groundspeed','angularvelocity','curvature'},type,ctrlresdata,resdata);
