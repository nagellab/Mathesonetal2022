function [genotable,genosidetable]=tabulateforLRclassifier(chosenfluor,chosenframerate,window)
%nocenterversion
totaldata=[];
dirs={};
sides={};
...
for k=1:numel(fn)%for each fly 
    ...
    temp=[];
    temp=[mean(mean(curronewind,2));mean(mean(currtwowind,2));mean(mean(currfourwind,2));mean(mean(currfivewind,2))];
