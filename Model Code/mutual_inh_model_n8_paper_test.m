function [heading_time, forward_signal_time, hDeltaC_output_time, u_time, local_output_time, pfl3r_heading_time, pfl3l_heading_time, pfl2_heading_time, pfl3r_time, pfl3l_time, pfl2_time] = mutual_inh_model_n8_paper_test(gain_wind, gain_inh, opto_pattern, wind_angle, heading_angle, t_pre, t_stim, t_post, allocentric)
%% Model Parameters
...
turnbase = 0.001; % baseline turn rate
tsigma = 20; % gain of random turn
vbase = 1.5; % Forward walking signal for pre-stimulus and post-stimulus periods

% Parameters for PFL2/3 activity
max_input = 1.3; % Maximum value of PFL3 heading bumps and hDeltaC wind bump
...
mutual_inh_thresh = 0.2; % threshold for activation function for mutual inhibition circuit
pfl_thresh = 2.3; % threshold for activation function for PFL2 and PFL3 neurons
...
if allocentric
    [hDeltaC_output, pfl3r_heading_input, pfl3l_heading_input, pfl2_heading_input] = heading_inputs_allocentric(N, heading_angle, wind_angle, max_input);
else
    [hDeltaC_output, pfl3r_heading_input, pfl3l_heading_input, pfl2_heading_input] = heading_inputs_pfn(N, heading_angle, wind_angle, max_input);
end
...
%     hDeltaC_output = phase_shift(hdc_col_mat * sigmoid_func(gain_wind * hDeltaC_input + opto_pattern, sigmoid_k, hdeltac_thresh)+ n_t);
    hDeltaC_output = phase_shift(hdc_col_mat * (gain_wind * hDeltaC_input + opto_pattern) + n_t);
